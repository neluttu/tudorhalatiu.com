...
$product = $db->query('SELECT id, products.name, products.slug, category, discount, sizes, weight, price, excerpt, description, status, stock, categories.name AS category_name, categories.slug AS category_slug FROM products LEFT JOIN categories ON categories.category_id = products.category WHERE id = :id', 
                    [
                        'id' => $params['id']
                    ])->findOrFail();
