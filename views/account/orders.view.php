...
<main class="flex flex-col items-start justify-start w-full gap-6 px-2 mt-2 md:mt-10 md:flex-row max-w-7xl">
    ...
    <div class="w-full md:grow">
        ...
        <? foreach($orders as $order) : ?>
            <div class="my-10">
                <span class="block mb-4 text-sm font-bold text-main-color">Comanda numarul #<?= str_pad($order['order_id'], 3, '0', STR_PAD_LEFT) ?> <span class="text-gray-500">din <?= roDate($order['ordered_at']) ?></span></span>
                <?
                ...
                <? } ?>
                <p class="w-full text-sm text-right text-main-color">Transport: <?= $order['shipping_tax'] ?> lei</p>
                <p class="w-full text-sm text-right text-main-color">Total: <?= number_format($total_price, 2, ',', '.') ?> lei</p>
            </div>
        <? endforeach ?>
    </div>
</main>
