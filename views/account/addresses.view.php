    ...
    <div class="w-full md:grow">
        ...
        <? endforeach; endif ?></div>
            ...
            <form id="checkout" class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3" method="post" action="/account/addresses">
                ...
                <div class="relative">
                    <input required type="text" name="email" id="email" value="<?= $billing['email'] ?>" placeholder="Adresa de email" class="w-full px-0 py-2 mt-1 border-b-2 <?= isset($errors['cart_email']) ? 'border-main-color shake-horizontal' : 'border-gray-200' ?> peer placeholder:text-transparent focus:border-gray-500 focus:outline-none" autocomplete="NA" />
