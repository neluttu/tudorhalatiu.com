<div class="w-full px-2 mx-auto mb-6 max-w-7xl">
    <h1 class="font-sans text-2xl md:text-4xl font-bold uppercase text-[#ed0078] relative inline-block slide-in-left">
        <!-- <div class="absolute -right-4 h-14 rounded-tr-md rounded-br-md border w-[1800px] top-0 -z-20"></div> -->
        <?= $heading ?>
    </h1>
    <p class="pl-10 mt-2 text-base italic text-slate-600"><?= $heading_info ?></p>
    <hr class="w-[100px] border-b-2 border-slate-700 mt-4">
</div>